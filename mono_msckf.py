...
from utils import *
from mono_feature import Feature
...
class IMUState(object):
    ...
    def __init__(self, new_id=None):
        ...
        # Transformation between the IMU and the camera (cam0)
...
class MSCKF(object):
    def __init__(self, config, run):
        ...
        self.imu_msg_buffer = []
        self.state_cov_buffer = np.zeros((1, 15))
        ...
        # Boxes 6DOF
        if self.run == 0:
            IMUState.gravity = np.array([0., 0., -9.807])
            self.state_server.imu_state.gyro_bias = np.array([0.049, 0.012, 0.002])
            self.state_server.imu_state.acc_bias = np.array([0.0275, -0.135, 0.14])
            self.state_server.imu_state.orientation = self.config.initial_orientation
            self.initial_rot = to_rotation(self.state_server.imu_state.orientation)

        # Event Flight
        elif self.run == 1:
            IMUState.gravity = np.array([0., 0., 9.807])
            self.state_server.imu_state.gyro_bias = np.array([0.029, 0.0075, -0.007])
            self.state_server.imu_state.acc_bias = np.array([-0.06, 0.28, 1.325])
            self.state_server.imu_state.orientation = self.config.initial_orientation
            self.initial_rot = to_rotation(np.array([0., 0., 0., 1.]))

        # Drone Racing
        elif self.run == 2:
            IMUState.gravity = np.array([0., 0., -9.807])
            self.state_server.imu_state.gyro_bias = np.array([-0.0617, -0.0033, -0.01983])
            self.state_server.imu_state.acc_bias = np.array([0.05, 0.4, 0.4])
            self.state_server.imu_state.orientation = self.config.initial_orientation
            self.initial_rot = to_rotation(self.state_server.imu_state.orientation)
    ...
    def batch_imu_processing(self, time_bound):
        ...
        for msg in self.imu_msg_buffer:
            ...
            if not self.initial_time:
                self.initial_time = imu_time
            self.state_cov_buffer = np.append(self.state_cov_buffer, np.reshape(np.diagonal(
                self.state_server.state_cov)[:15], (1, 15)), axis=0)
            self.timestamps = np.append(self.timestamps, imu_time - self.initial_time)
            self.position = np.append(self.position, np.reshape(self.initial_rot @ self.state_server.imu_state.position,
                                                                (1, 3)), axis=0)
            self.orientation = np.append(self.orientation, np.reshape(self.state_server.imu_state.orientation, (1, 4)), axis=0)
            self.velocity = np.append(self.velocity, np.reshape(self.initial_rot @ self.state_server.imu_state.velocity,
                                                                (1, 3)), axis=0)
            self.gyro_bias = np.append(self.gyro_bias, np.reshape(self.state_server.imu_state.gyro_bias, (1, 3)),
                                       axis=0)
            self.acc_bias = np.append(self.acc_bias, np.reshape(self.state_server.imu_state.acc_bias, (1, 3)), axis=0)
            used_imu_msg_count += 1
    ...
    def state_augmentation(self, time):
        ...
        self.state_cov_buffer[-1] = np.reshape(np.diagonal(self.state_server.state_cov)[:15], (1, 15))
    ...
    def add_feature_observations(self, feature_msg):
        ...
        for feature in feature_msg.features:
            if feature.id not in self.map_server:
                ...
                map_feature.observations[state_id] = np.array([feature.u0, feature.v0])
                self.map_server[feature.id] = map_feature
            else:
                # This is an old feature.
                self.map_server[feature.id].observations[state_id] = np.array([feature.u0, feature.v0])
    ...
    def measurement_update(self, H, r):
        ...
        state_cov = I_KH @ self.state_server.state_cov @ I_KH.T + (K @ K.T * self.config.observation_noise)
        ...
        self.state_cov_buffer[-1] = np.reshape(np.diagonal(self.state_server.state_cov)[:15], (1, 15))
        self.position[-1] = np.reshape(self.initial_rot @ imu_state.position, (1, 3))
        self.orientation[-1] = np.reshape(self.state_server.imu_state.orientation, (1, 4))
        self.velocity[-1] = np.reshape(self.initial_rot @ imu_state.velocity, (1, 3))
        self.gyro_bias[-1] = np.reshape(imu_state.gyro_bias, (1, 3))
        self.acc_bias[-1] = np.reshape(imu_state.acc_bias, (1, 3))
    ...
    def find_redundant_cam_states(self):
        ...
        for i in range(2):
            ...
            if angle < self.config.rotation_threshold and distance < self.config.translation_threshold and \
                    self.tracking_rate > self.config.tracking_rate_threshold:
    ...
    def publish(self, time):
        ...
        print('   orientation:', R_to_rpy(self.initial_rot @ to_rotation(imu_state.orientation).T) * 180/np.pi)
        print('   position:', imu_state.position)
        print('   velocity:', imu_state.velocity)
        print('   covariance:', np.sqrt(np.diagonal(self.state_server.state_cov)[:21]))
        print()
